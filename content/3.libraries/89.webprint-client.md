...
If you publish config file, you can set aliases and use them instead of UUID's in `GetPrinter`, `UpdatePromise`, `CreatePromise` and `CreatePromiseAndPrint` methods.
...
Headless printing, with using alias instead of uuid.
```php
